...
- (void)upgradeFirmware:(CDVInvokedUrlCommand *)command {
    ...
    NSString* fileUrlString = [obj valueForKey:@"fileUrl"];
    ...
    if (urlData)
    {
        ...
        NSArray  *paths = NSSearchPathForDirectoriesInDomains(NSDocumentDirectory, NSUserDomainMask, YES);
        NSString  *documentsDirectory = [paths objectAtIndex:0];
        NSString  *firmwarePath = [NSString stringWithFormat:@"%@/%@", documentsDirectory,@"fw.zip"];
        [urlData writeToFile:firmwarePath atomically:YES];
        NSLog(@"BLE filePath = %@", firmwarePath);
        if(firmwarePath!=nil){
            NSString  *localFirmwarePath = [NSString stringWithFormat:@"file://%@", firmwarePath];
            NSURL *localFirmwareUrl = [NSURL URLWithString:localFirmwarePath];
